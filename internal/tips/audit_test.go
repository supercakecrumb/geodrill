...
// auditDecks mirrors tipDecks (data.go): only decks with a curated,
...
// deck's languages here (and to tells in data.go) once that group's cue set
// has been mined and hand-filtered.
...
//	  go test ./internal/tips/ -run TestAuditTellsAgainstCorpus -v
