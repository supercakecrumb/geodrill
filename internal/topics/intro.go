...
// it translates candidates (already ordered tier-then-topic-then-item
// position — the priority order architecture §1.4/§4.2 expects the caller to
// supply) into engram.IntroItem, applies the engine's daily-budget and
// New-only filter, and translates the winners back to storage.Item —
// preserving the caller's order. introducedToday is how many intros were
