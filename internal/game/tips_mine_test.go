...
// tips_mine_test.go is the reusable corpus miner used to bootstrap each
// deck's cue list during the staged tips rollout: it finds, per language,
// words that are frequent in that language and near-absent in every deck
// sibling — candidates for word cues. Results are printed for hand-filtering
// (drop proper nouns and corpus artifacts) before the survivors are added to
// tips_data.go's tells map. It iterates auditDecks (see tips_audit_test.go),
// so re-running it against a newly curated deck picks up that deck's
// languages automatically.
// Run: TIPS_AUDIT_DATABASE_URL=... go test ./internal/game/ -run TestMine -v
