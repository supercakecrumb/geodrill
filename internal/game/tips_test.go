...
// deckLanguages mirrors the decks in tipDecks (tips_data.go) — every
// language in a rolled-out deck must have dataset coverage.
...
// TestTipFor_MatchesTheMissedLanguage covers the game's own entry point
// (design doc "Language Roulette": "the language tip for the missed
// language when one exists") — it must behave exactly like build with
// Correct: false and no chosen-answer fields, since a tip is only ever
// shown after a WRONG guess ends the run.
func TestTipFor_MatchesTheMissedLanguage(t *testing.T) {
	tip := TipFor("ita", "Italian", "È un'altra questione.")
	if tip != "Italian: “è” means “is”" {
		t.Fatalf("TipFor = %q", tip)
	}
}

func TestTipFor_NoMatchMeansNoTip(t *testing.T) {
	if tip := TipFor("por", "Portuguese", "Falta pouco."); tip != "" {
		t.Fatalf("TipFor = %q, want empty (nothing in the sentence to point at)", tip)
	}
}
