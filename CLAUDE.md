...
- **Unit tests run against any DB; integration tests need a disposable "test" DB.** Integration tests (`GEODRILL_TEST_DATABASE_URL`) run `freshSchema`, which **drops every table** — so their DB name must contain "test" (`testDSN` code fuse). Never point them at the live/dev DB; doing so wiped it once (2026-07-15). `pg_dump` the dev DB before any destructive schema work.
