-- geodrill v2 core schema (architecture contract vibe/geodrill-v2-architecture.md
-- §2.1-2.4, 2.6-2.10). Adds the topic/item framework alongside the existing
-- decks/skills tables (dropped later in 000007, after the old-quiz data
-- migration backfills item_id). Order matters for FK resolution: countries
-- before items, topics before items/user_topics, items before user_items /
-- introductions, fact_defs before country_facts.

-- 2.1 Topic tree (parent_id + slug; topic_paths view below for subtree reads).
CREATE TABLE topics (
...
-- 2.3 Per-user per-item lifecycle + FSRS card (replaces user_skills for v2 topics).
...
-- 2.8 Media (photos: local path + telegram file_id cache). content_items
-- already exists (000001); its topic_id column is added in 000006.
...
-- 2.10 Users/settings — keep reminders, add intro cap; optional per-user topic opt-in/out.
ALTER TABLE users
...
-- 2.9 Tiers + global gating — per-user completion cache (recomputed
